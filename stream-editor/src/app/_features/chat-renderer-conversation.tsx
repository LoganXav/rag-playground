...
import { applyEditsToMarkdown } from "@/lib/structured-test/apply-edits";
...
export default function ChatRendererConversation({
  ...
  setMessages: (
    updater: ((prev: ChatMessage[]) => ChatMessage[]) | ChatMessage[],
  ) => void;
}) {
            ...
            <div className="text-sm leading-relaxed whitespace-pre-wrap prose prose-invert max-w-none">
              <ReactMarkdown
                ...
                components={{
                  ...
                  ol: ({ children }) => (
                    <ol className="m-0 list-decimal list-inside">{children}</ol>
                  ),
                  li: ({ children }) => (
                    <li className="m-0 leading-tight">{children}</li>
                  ),
                }}
              >
                {msg.content.replace(/\n/g, "  \n")}
              ...
              <div className="mt-4">
                <ChatRendererConversationPreview
                  ...
                  onDismiss={() =>
                    setMessages((prev) =>
                      prev.map((m) =>
                        m.id === msg.id ? { ...m, preview: undefined } : m,
                      ),
                    )
                  }
                />
